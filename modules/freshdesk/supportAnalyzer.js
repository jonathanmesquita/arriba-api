...
function inferProduct(text) {
  ...
  if (/datacob|data cob|cobranca|renegociacao|negociacao|parcela|parcelamento|mailing|recepcao|contrato|boleto|versao|versão|checklist versao|agendamento de versao|virada de versao/.test(text)) return "DataCob";
...
function inferFreshdeskType(text) {
  if (hasVersionIntent(text)) return "Versao";
...
function hasVersionIntent(text) {
  return /versao|versão|atualizacao de versao|atualização de versão|agendamento de versao|agendamento de versão|agendamento automatico|agendamento automático|checklist versao|checklist versão|virada de versao|virada de versão|homologacao|homologação/.test(text);
}

function hasKnowledgeType(matches = [], type = "") {
...
function inferRoutine(text) {
  if (hasVersionIntent(text)) return "Agendamento automatico de versao";
...
function hydrateAnalysis(analysis, ticket = {}, conversations = [], context = {}) {
  ...
  const product = hasVersionKnowledge
    ? "DataCob"
    : (analysis.product && analysis.product !== "Nao identificado" ? analysis.product : inferProduct(rawText));
  const inferredType = hasVersionKnowledge ? "Versao" : inferFreshdeskTypeStrong(rawText, analysis.freshdeskType || inferFreshdeskType(rawText));
  let freshdeskType = ensureAllowedFreshdeskType(inferredType);
  if (hasReportIntent(rawText) && !hasCommercialIntent(rawText) && !hasVersionKnowledge) freshdeskType = "Relatorio";
  ...
  if (freshdeskType === "Versao") recommendedScenario = "Mover para Datacob";
  ...
  const initialNeedsDevelopmentSpec = freshdeskType === "Versao" ? false : Boolean(
  ...
  const baseChecklist = Array.isArray(analysis.checklist) && analysis.checklist.length ? analysis.checklist : fallbackChecklist;
  const checklist = mergeKnowledgeChecklist(baseChecklist, knowledgeMatches);
  ...
  const hydrated = {
    ...
    routine: primaryKnowledge?.freshdeskType === "Versao" ? "Agendamento automatico de versao" : (analysis.routine || inferRoutine(rawText)),
    summary: analysis.summary || (primaryKnowledge ? `${primaryKnowledge.summary}` : `Chamado relacionado a ${product}. Classificacao inicial: ${freshdeskType}. Prioridade sugerida: ${priority}.`),
    currentScenario: analysis.currentScenario || (primaryKnowledge ? `Ticket relacionado a base de conhecimento: ${primaryKnowledge.title}. ${primaryKnowledge.summary}` : `O cliente abriu um chamado com indicios de ${requestType.toLowerCase()} relacionado a ${analysis.routine || inferRoutine(rawText)}. E necessario revisar a descricao e as evidencias anexadas antes de concluir o encaminhamento.`),
    expectedBehavior: analysis.expectedBehavior || (primaryKnowledge ? `Orientar o cliente conforme o artigo/manual recomendado e validar as regras operacionais antes de concluir o atendimento.` : "O atendimento deve identificar a causa, orientar o cliente ou encaminhar a demanda com contexto suficiente para analise tecnica."),
    suggestedReply: analysis.suggestedReply || primaryKnowledge?.suggestedReply || "Agradecemos por sua mensagem e queremos informar que ja estamos analisando sua solicitacao. Para agilizar a analise, poderia nos encaminhar prints, exemplos, passo a passo realizado e demais evidencias relacionadas ao cenario informado?",
    checklist,
    ...
    knowledgeSummary: summarizeKnowledgeMatches(knowledgeMatches),
    nextAction: analysis.nextAction || primaryKnowledge?.nextAction || "Revisar a analise, confirmar evidencias faltantes e escolher a resposta predefinida mais adequada."
  };
...
- Quando o assunto mencionar versao, atualizacao de versao, checklist versao, homologacao, producao, virada de versao ou agendamento automatico, classifique como product DataCob e freshdeskType Versao.
...
Base de conhecimento local relacionada:
${summarizeKnowledgeMatches(findKnowledgeMatches(ticket, conversations, context))}`
        }
      ]
    });
